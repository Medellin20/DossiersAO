    ...
    try {
      ...
      try {
        ...
        if (added.length || removed.length) {
          if (added.length) changes.push(`assignée à ${added.join(", ")}`);
          if (removed.length)
            changes.push(`désassignée de ${removed.join(", ")}`);
        }
        ...
        if (added.length || removed.length) {
          for (const mId of added) {
            NotificationService.broadcast(
              "task_assigned",
              "Tâche assignée",
              `DAO ${dao.numeroListe} – Tâche #${task.id} (${task.name}) assignée à ${mId}`,
          ...
          for (const mId of removed) {
            NotificationService.broadcast(
              "task_unassigned",
              "Tâche désassignée",
              `DAO ${dao.numeroListe} – Tâche #${task.id} (${task.name}) désassignée de ${mId}`,
              { daoId: id, taskId: task.id, assignedTo: task.assignedTo },
            );
          }
        } else {
